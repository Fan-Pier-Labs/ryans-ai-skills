...
Eleven Claude Code skills, in four groups:
...
- **Dependencies are settled before the work starts**: a skill that needs a tool it
  cannot run without — an analyzer, a secret scanner, Playwright, a language
  toolchain — detects what is missing first, asks **once** with the whole list and
  the exact install command, and then either installs and runs or **stops**. It
  never starts and stalls halfway, and never substitutes a grep or a column of
  Unknowns for a tool it was told not to install. Audit skills default to the
  ephemeral install (`npx --yes`, `uvx`) so the repo stays untouched; anything that
  lands in `package.json` is named as such in the ask. The contract is
  [`skills/shared/dependency-preflight.md`](skills/shared/dependency-preflight.md)
  and it covers code-quality-audit, sec-ops-audit, pr-demo-media, dependency-updater
  and enable-more-lint-or-ts-checks. `gh`, `aws` and an SSH key are *access*, not
  packages, and stay in each skill's own scope step.
- **One copy of shared code**: [`skills/shared/`](skills/shared/README.md) is not
  ...
  `--reviews-are-feedback`), `ec2-ssh-sweep.sh`, and `dependency-preflight.md`
  (the one contract behind the bullet above — a rule, not a script). Skills reach it by a
...
- **Write surfaces**: pr-demo-media posts comments only; auto-reviewer posts
  ...
  one branch and opens one PR on this repo only; code-quality-audit writes one
  report file, and opens an issue only when asked. Nothing merges, force-pushes,
  or deletes.
