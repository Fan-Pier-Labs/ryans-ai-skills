...
Eleven Claude Code skills. Eight operate on GitHub — six standalone coding agents plus a
...
run it once when taking over a codebase, then let auto-reviewer hold the line PR by PR.
enable-more-lint-or-ts-checks is what you run on the findings: code-quality-audit reports
which of its 105-rule ESLint baseline is off, and this skill measures those rules and turns
the affordable ones on, one PR at a time. pr-watcher
