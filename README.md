...
Six Claude Code skills. Four are standalone coding agents operating on the
`Fan-Pier-Labs` and `ryanhugh` GitHub accounts; two review cloud infrastructure (cost, and
...
| [infra-audit](infra-audit/SKILL.md) | Audits a startup's infra against twelve questions — **append-only CloudTrail** (Object Lock COMPLIANCE, 14-rule PASS/FAIL table modelled on the Fan Pier Labs trail), what breaks first at scale, DB backups, self-hosted DB, right infra for the use case, split hosting, too many providers, 2FA enforced everywhere, RDS rotating password, crash/OOM resilience, observability, mainstream vs budget hosting — plus security hygiene found on the way. Read-only `scripts/aws-audit-inventory.sh` + the same SSH sweep with an audit collector (patching, ports, users, secret *names* only, backups, startup config). Scorecard report | on demand |
| [pr-demo-media](pr-demo-media/SKILL.md) | Demos frontend PRs: records a Playwright video or captures before/after screenshots — whichever fits the change — and posts it on the PR via gh-attach | continuous loop, ~20–30 min |
...
- **Env tuning**: `OWNERS`, `DAYS`, `MARKER` on every script.
