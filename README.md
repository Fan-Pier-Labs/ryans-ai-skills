...
| [pr-watcher](pr-watcher/SKILL.md) | Runs any of the PR-reactive skills the moment a PR changes: `/pr-watcher run /auto-reviewer /ci-runner`. One `gh webhook forward` per repo (polling if the repo won't grant a webhook) feeds a queue; the agent drains it with a capped number of subagents, one per PR. The watcher itself writes nothing to GitHub | while the session runs |
