...
- `scripts/ec2-ssh-sweep.sh` — `discover` running instances, then `run` a collector on each over SSH or SSM; §3. A symlink to `skills/shared/ec2-ssh-sweep.sh`, shared with infra-audit and sec-ops-audit.
