...
- `scripts/ec2-ssh-sweep.sh` — `discover` running instances, then `run` the collector on each over SSH or SSM; §3. A symlink to `skills/shared/ec2-ssh-sweep.sh`, shared with infra-cost-review and sec-ops.
